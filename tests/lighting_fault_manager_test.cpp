...
public:
    bool read_current_ma(bike::LightingOutput output, std::uint16_t& current_ma) const override {
        const auto i = FakeHardware::index(output);
...
int main() {
    ...
    assert(brake.latched);
    assert(brake.latched_status == bike::LightingElectricalStatus::OpenLoad);
    assert(brake.occurrence_count == 1);
    assert(brake.first_seen_ms == 300);
    assert(hardware.read_output(bike::LightingOutput::BrakeBright)); // report-only default

    assert(faults.clear_latch(bike::LightingOutput::BrakeBright));
    assert(!faults.record(bike::LightingOutput::BrakeBright).latched);

    hardware.write_output(bike::LightingOutput::LeftIndicator, true);
    feedback.current[0] = 6000; // over-current
    faults.set_policy(bike::LightingOutput::LeftIndicator,
                      bike::LightingFaultPolicy::ShutdownOnOverCurrent);
    faults.service(400);
    faults.service(500);
    faults.service(600);
    const auto& left = faults.record(bike::LightingOutput::LeftIndicator);
    assert(left.latched);
    assert(left.latched_status == bike::LightingElectricalStatus::OverCurrent);
    assert(left.shutdown_applied);
    assert(!hardware.read_output(bike::LightingOutput::LeftIndicator));
