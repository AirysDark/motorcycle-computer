...
enum class InputPolarity : std::uint8_t { ActiveLow, ActiveHigh };
...
public:
    ...
    bool write(const std::uint8_t* data, std::size_t length) override { return serial_.write(data, length) == length; }
...
private:
    static void configure_output(int pin) { digitalWrite(pin, LOW); pinMode(pin, OUTPUT); digitalWrite(pin, LOW); }
...
public:
    ...
    bool left_indicator_requested() const override { return active(kLeftSwitchPin, kLeftSwitchPolarity); }
    bool right_indicator_requested() const override { return active(kRightSwitchPin, kRightSwitchPolarity); }
    bool front_brake_active() const override { return active(kFrontBrakeInputPin, kFrontBrakePolarity); }
    bool rear_brake_active() const override { return active(kRearBrakeInputPin, kRearBrakePolarity); }
    bool high_beam_requested() const override { return active(kHighBeamInputPin, kHighBeamSwitchPolarity); }
private:
    static void configure_input(int pin, InputPolarity polarity) { pinMode(pin, polarity == InputPolarity::ActiveLow ? INPUT_PULLUP : INPUT_PULLDOWN); }
...
// Placeholder until current-sense hardware is selected. It deliberately reports
// feedback unavailable rather than inventing a current measurement.
class NoLightingElectricalFeedback final : public bike::LightingElectricalFeedback {
public:
    bool read_current_ma(bike::LightingOutput, std::uint16_t& current_ma) const override {
        current_ma = 0;
        return false;
    }
};
